...
from django_fido.management.commands.download_authenticator_metadata import (
    InvalidCert,
    _get_metadata,
    _prepare_crypto_store,
    verify_certificate,
)
...
class TestVerifyCertificate(SimpleTestCase):
    ...
    @override_settings(
        DJANGO_FIDO_METADATA_SERVICE={"ACCESS_TOKEN": "secret_token", "DISABLE_CERT_VERIFICATION": False}
    )
    ...
    @override_settings(
        DJANGO_FIDO_METADATA_SERVICE={
            "ACCESS_TOKEN": "secret_token",
            "CERTIFICATE": [os.path.join(DIR_PATH, "Root_bad.cer"), os.path.join(DIR_PATH, "Root.cer")],
        }
    )
    ...
    def test_malformed_key(self):
        token = (
            "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCIsIng1YyI6WyJhYWFhYWFhIl19.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6Ikp"
...
@override_settings(
    DJANGO_FIDO_METADATA_SERVICE={"ACCESS_TOKEN": "secret_token", "CERTIFICATE": [os.path.join(DIR_PATH, "Root.cer")]}
)
@patch("django_fido.management.commands.download_authenticator_metadata._prepare_crypto_store", prepare_timed_context)
...
class TestDownloadAuthenticatorMetadata(TestCase):
    ...
    def test_aaid(self, get_metada_patch):
        entry = {
            "aaid": "1234#5678",
            "url": "https://example.com/1234abcd",
        ...
        payload = {
            ...
            "userVerificationDetails": [
                [
                    {
                        "userVerification": 2,
                        "baDesc": {"FAR": 0.00002, "maxRetries": 5, "blockSlowdown": 30, "maxReferenceDataSets": 5},
                    }
                ]
            ],
            "keyProtection": 6,
    ...
    def test_key_identifiers(self, get_metada_patch):
        entry = {
            "aaguid": "7c0903708b87115b0b422def3138c3c864e44573",
        ...
        with responses.RequestsMock() as rsps:
            rsps.add(
                responses.GET,
                "https://example.com/7c0903708b87115b0b422def3138c3c864e44573",
                body=urlsafe_b64encode(json.dumps(payload, sort_keys=True).encode()),
            )
            call_command("download_authenticator_metadata")
    ...
    def test_hash_mismatch(self, get_metada_patch):
        ...
        metadata = AuthenticatorMetadata.objects.get(identifier="7c0903708b87115b0b422def3138c3c864e44573")
        self.assertEqual(
            "Hash invalid for authenticator ['7c0903708b87115b0b422def3138c3c864e44573'].\n", output.getvalue()
        )
        self.assertJSONEqual(metadata.metadata_entry, entry)
